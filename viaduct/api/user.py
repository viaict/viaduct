...
class UserAPI:
	...
	def get_groups_for_current_user():
		...
		If there is no current_user (no sign in), guests is returned if guests exists,
		otherwise it crashes because there can not be no guests
		...
		if current_user == None:
			group = Group.query.filter(Group.name=='guests').first()
			if not(group):
