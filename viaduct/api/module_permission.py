...
class ModulePermissionAPI:
	...
	def can_read(module_name):
		...
		"""
		return ModulePermissionAPI.get_highest_permission_for_module(module_name) >= 1
	...
	def can_write(module_name):
		...
		"""
		return ModulePermissionAPI.get_highest_permission_for_module(module_name) >= 2
	...
	def get_highest_permission_for_module(module_name):
		...
		highest = 0
		for group in groups:
			query = ModulePermission.query.filter(ModulePermission.group_id==group.id)
			query = query.filter(ModulePermission.module_name==module_name)
