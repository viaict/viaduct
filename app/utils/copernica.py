...
@copernica_failsafe
def update_newsletter(user_id, subscribe=True):
    copernica_id = via_id_to_copernica_id(user_id)
    if not copernica_id:
        return
...
@copernica_failsafe
def via_id_to_copernica_id(user_id):
...
@copernica_failsafe
def update_user(websiteID, mail, first, last, study, studentnr,
               *args, **kwargs):
    copernica_id = via_id_to_copernica_id(websiteID)
    if not copernica_id:
        print("Unknown user, creating new user in copernica.")
