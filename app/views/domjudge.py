...
def contest_view(contest_id, page):
    link = False
    if role_service.user_has_role(Roles.DOMJUDGE_WRITE):
        link = True
...
def contest_submissions_view(contest_id, team_id=None):
    ...
    if team_id and not role_service.user_has_role(Roles.DOMJUDGE_WRITE):
        return abort(403)
...
def render_contest_submissions_view(contest_id, view_all=False, team_id=None):
    admin = role_service.user_has_role(Roles.DOMJUDGE_WRITE)
